...
public class CDHInventoryPC13Test {
	...
	public void setup() throws Exception {
		CDH_URL_US = reader.getData("cdh-inventory-US-pc13");
		CDH_URL_CA = reader.getData("cdh-inventory-CA-pc13");
		testDataMap = TestDataUtils.readFromFileAndConvertToMap(new File("src/test/resource/testdata/cdhinventory.csv"));
		
			htmlReporter = new ExtentHtmlReporter(new File(System.getProperty("user.dir")+"/target/CDHAPIAutomationReportPC13.html"));
	...
	@Test
	public void checkPC13() {
		
		for(Map<String,String> testData : testDataMap) {
			testInfo = report.createTest("Test case : CheckPC13");
			...
			if (regionUS.toLowerCase().contains("us"))
				...
				System.out.println(regionUS);
				 resp = given(). 
						pathParam("pc13", testData.get("PC13")).
			...
			System.out.println("Print the Message of the pc13 : " + responseMessage);
			
			List<String> codeOfThepc13 = resp.
					then().extract().path("inventory.code");
			System.out.println("Check the Inventory code : " + codeOfThepc13.get(0));
			...
			if(inventory.get(0)>5)
			{
				assertEquals(stockStatus.get(0), "In Stock");
				System.out.println(inventory);
			} else  if (inventory.get(0)<=5 && inventory.get(0)>=1)
			{
			...
			testInfo.log(Status.PASS, "The Actual PC13 is : " + actualCode);
	...
	public void captureStatus(ITestResult result) {
		if (result.getStatus() == ITestResult.SUCCESS) {
			...
			testInfo.log(Status.FAIL, "The Actual PC13 is : " + actualCode);
